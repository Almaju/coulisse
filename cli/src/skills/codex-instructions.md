# Coulisse configuration

Coulisse is the local OpenAI-compatible gateway for this project, configured
entirely through `coulisse.yaml`. When asked to add, change, or remove
Coulisse configuration, read the file first, make the change, then validate:
...
- Agent and experiment names share a namespace — they must be unique.
- `coulisse check` validates the full graph without starting the server.
