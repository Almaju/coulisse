# Summary

[Introduction](./introduction.md)

# Getting Started

- [Installation](./getting-started/installation.md)
- [Your first config](./getting-started/first-config.md)
- [Making a request](./getting-started/first-request.md)

# Essentials

The handful of things every Coulisse setup touches. If you only read one
section, read this one — providers, agents, memory, and tools cover the
common case.

- [Providers](./configuration/providers.md)
- [Agents](./configuration/agents.md)
- [Memory](./configuration/memory.md)
- [MCP tools](./configuration/mcp.md)
- [Multi-agent routing](./features/multi-agent.md)
- [MCP tool integration](./features/tools.md)
- [Multi-backend support](./features/backends.md)
- [Streaming responses](./features/streaming.md)
- [Studio UI](./features/studio-ui.md)

# Advanced

Opt-in features for production deployments, evaluation, and orchestration.
Skip until you need them — none are required for a working server.

- [User identification](./configuration/user-id.md)
- [API tokens](./features/api-tokens.md)
- [Rate limiting](./features/rate-limiting.md)
- [Per-user OAuth for MCP](./features/mcp-oauth.md)
- [Per-user memory (deep dive)](./features/memory.md)
- [File attachments](./features/attachments.md)
- [Structured outputs](./features/structured-output.md)
- [Response language](./features/language.md)
- [Token cost tracking](./features/pricing.md)
- [Skills](./features/skills.md)
- [Async tasks](./features/async-tasks.md)
- [Triggers (cron, webhook)](./features/triggers.md)
- [Sidecars](./features/sidecars.md)
- [LLM-as-judge evaluation](./features/evaluation.md)
- [Experiments (A/B testing)](./features/experiments.md)
- [Smoke tests](./features/smoke-tests.md)
- [Telemetry configuration](./configuration/telemetry.md)
- [Telemetry & OpenTelemetry](./features/telemetry.md)

# Reference

- [CLI](./reference/cli.md)
- [HTTP API](./reference/api.md)
- [YAML schema](./reference/yaml.md)
- [Releasing](./reference/releasing.md)
- [Roadmap](./reference/roadmap.md)
